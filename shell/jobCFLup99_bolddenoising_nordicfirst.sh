...
do
  old_filename=${subject}_ses-${session}_task-${suffix}_bold #${subject}_ses-${session}_task-${suffix}_desc-preproc_bold
  new_filename=${subject}_ses-${session}_task-${suffix}_desc-nordic_bold
  cp ${PROJ_DIR}/${subject}/ses-${session}/orig/${old_filename}.nii.gz ${PROJ_DIR}/${subject}/ses-${session}/func/${new_filename}.nii.gz
...
master -m 10 -s $1 --sge -q short.q #Nordic, --mag magnitude only
