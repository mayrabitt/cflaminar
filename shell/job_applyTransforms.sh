...
#antsApplyTransforms --interpolation BSpline[5] -d 3 -i ${masked_T1w} -r ${masked_T1w} -o ${COREG_DIR}/${subject}_ses-${session}_acq-MP2RAGE_desc-masked_T1wAffine.nii.gz -t ${outputPrefix}0GenericAffine.mat #forward transform
cp ${COREG_DIR}/${subject}_ses-${session}_acq-MP2RAGE_desc-masked_T1wWarped.nii.gz ${PROJ_DIR}/derivatives/masked_mp2rage/${subject}/ses-${session}/anat/${subject}_ses-${session}_acq-MP2RAGE_desc-masked_T1w.nii.gz

#Transform T2w

antsApplyTransforms --interpolation BSpline[5] -d 3 -i ${T2w} -r ${T2w} -o ${COREG_DIR}/${subject}_ses-${session}_acq-3DTSE_T2wWarped.nii.gz -t ${outputPrefix}1Warp.nii.gz -t ${outputPrefix}0GenericAffine.mat
